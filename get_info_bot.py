...
# использование not call messages (text)
@bot.message_handler(commands=['menu'])
def menu_buttons2(message):
    markup = create_menu(message.chat.id)
    bot.send_message(message.chat.id, text='Меню:', reply_markup=markup)

# использование call messages
@bot.callback_query_handler(func=lambda call: call.data == "menu")
def menu_buttons(call):
    markup = create_menu(call.message.chat.id)
    bot.send_message(
...
def handle_text(message):
    ...
    def filter_time(text):
        ...
        # смена точки на двоеточие
        filter_time_text = text.replace('.', ':')
    ...
    action = True
    def should_it_send_error_message():
        action_f = action
        return action_f
    ...
    # пользователь написал время неправильно
        ...
        if location_data[0]:
            bot.send_message(chat_id=message.chat.id,
                             text='Место было успешно добавлено!'
            )
            ...
            # отправка данных пользователю после лобавления места
            bot.send_message(chat_id=message.chat.id, text=f'{format_f(id_tg_)}')
            time.sleep(10)

            # возвращение в меню
            menu_buttons2(message)
        ...
        if should_it_send_error_message():
            bot.send_message(
                chat_id=message.chat.id,
                text='Извините я вас не понимаю. Мой создатель не наделил меня способностью поддерживать диалог.\n'
                    ...
                    'Например: 07:10, 23:59, 00:00, 13.59, 23.35\n\n'
                    'Если вы хотели ввести место для отправки данных и вам пришло это сообщение, к сожалению данное местоположения в базе данных отсутствует. '
            )
    # обновление словаря
